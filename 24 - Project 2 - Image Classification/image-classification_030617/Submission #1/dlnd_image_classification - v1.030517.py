...
def conv2d_maxpool(x_tensor, conv_num_outputs, conv_ksize, conv_strides, pool_ksize, pool_strides):
    ...
    :param conv_ksize: kernal size 2-D Tuptf.Variable(tf.random_normal([5, 5, 1, 32]))for convolution
...
def conv_net(x, keep_prob):
    ...
    conv_pool1 = conv2d_maxpool(x, 128, [4,4], [1,1], [4,4], [1,1])
    dropout1   = tf.nn.dropout(conv_pool1,keep_prob) # Try drop out here
    conv_pool2 = conv2d_maxpool(dropout1, 128, [2,2], [1,1], [2,2], [1,1])
    dropout2   = tf.nn.dropout(conv_pool2,keep_prob) # Try drop out here
    conv_pool3 = conv2d_maxpool(dropout2, 128, [1,1], [1,1], [1,1], [1,1]) # 1x1 convolution!
    dropout3   = tf.nn.dropout(conv_pool3,keep_prob) # Try drop out here
    ...
    fullyconn = fully_conn(flattened, 32)
    dropout4 = tf.nn.dropout(fullyconn,keep_prob) # Try drop out here
...
# With batch_size set to 64, I get a ResourceExhaustedError when I run Fully Train the Model
# (at the end when an attempt to save the model is made). So I will try a smaller batch size.
# Still getting a ResourceExhaustedError even with batch_size set to 32.
# Maybe I need to try this: https://nd101.slack.com/archives/project-2/p1488478718009063
epochs = 25
batch_size = 32
